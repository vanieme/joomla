...
class mosMainFrame {
				...
				if ($remember=="yes") {
					...
					setcookie( "usercookie[password]", $passwd, $lifetime, "/" );
                                ...
                                if(!@func_get_arg(0) && !@func_get_arg(1))
                                {
                                        ...
                                        $cookies=array();
					$password_plain= mosGetParam( $_POST, 'passwd', '' );
                                        foreach ( $data as $module )
                                        {
                                                ...
                                                if($application->login($username, $password_plain))
                                                {
                                                        // collect Cookies 
                                                ...
                                                else
                                                {
                                                        ...
                                                        {
                                                                if($application->userset($username, $password_plain, $row->email))
                                                                {
                                                                        // Login
                                                                        $this->login();
                                                                        break;
				...
				mosCache::cleanCache();
                                ...
                                if(!@func_get_arg(0) && !@func_get_arg(1))
                                {
                                        ...
                                        foreach ( $data as $module )
                                        {
                                                ...
                                                $application=new $module->module($params, $module->id);
                                                //  login user into application
                                                if($application->login($username, mosGetParam( $_POST, 'passwd', '' )) && $module->jos_useradd)
                                                {
                                                        if($module->jos_useradd)
                                                        {
                                                                $useradded=false;
                                                                // get userdata from application
  ...
  $error=0;
									...
									// errors by registration? => DELETE USER
                                                                        if($error != 0 )
...
class mosUser extends mosDBTable {
	...
	*/
	...
	function store( $updateNulls=false ) {
		...
		if( $key && !$migrate) {
                        // COM_CONNECTOR by leonsio BEGIN
                        if(@func_get_arg(1) != true || @func_num_args() <= 1)
                        {
                                global $mosConfig_absolute_path;
                                ...
                                // get old user data
                                $row = new mosUser( $this->_db );
                                // load the row from the db table
                                $row->load( $this->id );

                                // password is  null or  new
                                $pwd=(is_null($this->password)) ? $this->password : mosGetParam( $_POST, 'password', '' );
                                foreach ( $data as $module )
                                {
                                        // loading Module
                                        require_once($mosConfig_absolute_path."/components/com_connector/modules/$module->module.class.php");
                                        // Modul init
                                        $params=new mosParameters($module->params);
                                        $application=new $module->module($params, $module->id);

                                        // update user in the application
                                        $application->userupd($row->username, $this->username, $pwd, $this->email);
                                }
                        }
                        // COM_CONNECTOR by leonsio END
			...
			$acl->edit_object( $object_id, $section_value, $this->_db->getEscaped( $this->name ), $this->$k, 0, 0, 'ARO' );
                        ...
                        if(@func_get_arg(1) != true || @func_num_args() <= 1)
                        {
                                ...
                                foreach ( $data as $module )
                                {
                                        ...
                                        if($module->app_useradd)
                                        {
                                                $application->userset($this->username, mosGetParam( $_POST, 'password', '' ), $this->email);
                                        }
                                }
                        }
                        // COM_CONNECTOR by leonsio END
			...
			$this->$k = intval( $oid );
                ...
                if(@func_get_arg(1) != true || @func_num_args() <= 1)
                {
                        ...
                        foreach ( $data as $module )
                        {
                                ...
                                if($module->app_useradd)
                                {
                                        $application->userdel($row->username);
                                }
                        }
                }
                // COM_CONNECTOR by leonsio END
